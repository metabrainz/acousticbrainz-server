      ...
      if (hash.length > 0) {
        ...
        for (let i = 0; i < this.state.jobs.length; ++i) {
          if (this.state.jobs[i].id == hash) {
            ...
            console.debug(
              `Found job with a specified ID (${hash}). ` + `Switching view.`
            );
            return;
          }
        }
...
class JobDeleteButton extends React.Component {
  ...
  render() {
    ...
    return (
      <div>
        <em>Are you sure?&nbsp;&nbsp;</em>
        <a href="#" className="btn btn-danger btn-xs" onClick={this.delete}>
          Delete
        </a>
        <a href="#" className="btn btn-default btn-xs" onClick={this.cancel}>
          Cancel
        </a>
      </div>
    );
        ...
        if (this.props.outdated) {
          status = (
            <div className="alert alert-success">
              The dataset has been changed since this job was run, so the
